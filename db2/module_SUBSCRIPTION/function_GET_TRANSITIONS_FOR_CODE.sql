--------------------------------------------------------------------------------------------------------------------------------
-- File:        function_GET_TRANSITIONS_FOR_CODE.sql
--              (c) Copyright Jeremy Rickard 2025
--------------------------------------------------------------------------------------------------------------------------------

ALTER MODULE subscription
ADD FUNCTION get_transitions_for_code(p_subscription_id INTEGER, p_subscription subscription_row)
  RETURNS TABLE
  (
  ...
  SET v_max_transition_id =
    (
      ...
      WHERE
        ...
        transition_code = p_subscription.transition_code AND
        transition_id > p_subscription.last_transition_id AND
        transition_utc_ts < v_utc_ts
      WITH CS
    );

  -- Return subscription transition information for specified transition code.
  FOR r AS
    SELECT
      t.transition_id,
      t.object_type_id,
    ...
    WHERE
      ...
      t.transition_code = p_subscription.transition_code AND
